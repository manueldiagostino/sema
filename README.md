...
A Next.js 16 platform for exploring and managing medieval charter corpora encoded in TEI P5 XML. Browse a public corpus table and entity knowledge graph, or use the admin dashboard to create, edit, and delete TEI documents.

## Features

- **Public corpus explorer** — searchable table of charters with an interactive entity knowledge graph
- **Admin dashboard** — area for document management (create, edit, delete)
...
Parse the TEI XML files in `data/corpus/` and generate the JSON files the app needs:
...
Open [http://localhost:3000](http://localhost:3000) for the public corpus explorer, or [http://localhost:3000/admin](http://localhost:3000/admin) for the admin dashboard. No authentication is required in local development.
...
This creates `data/fake/` with 100 fake charters, then rebuilds the JSON artifacts. The app automatically detects `data/fake/` and uses it instead of `data/corpus/` — no config changes needed.
...
The admin panel saves new and edited TEI documents to `data/corpus/`. To share these changes with other users (e.g. push to a shared GitHub repository):
...
**Quick start:** double-click `start.bat`. Make sure Node.js is installed first.

## Project Structure

```
config/                  YAML configuration (form fields, corpus columns)
data/corpus/             TEI P5 XML source files
